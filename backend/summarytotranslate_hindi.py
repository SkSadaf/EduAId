...
def translate_to_hindi(text):
    prompt = f"""
    Translate the following English text to Hindi. Keep the translation natural and maintain the paragraph structure:

    Text to translate:
    '{text}'

    Please provide only the Hindi translation without any additional explanations.
    """
...
def process_transcript(transcript_text):
    ...
    return summary, hindi_summary, mcqs
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            if transcript_text:
                summary, hindi_summary, mcqs = process_transcript(transcript_text)
                ...
                if save_choice.lower() == 'y':
                    ...
                    # Save Hindi summary
                    hindi_filename = f"summary_hindi_{video_id}.txt"
                    save_to_file(hindi_summary, hindi_filename)
                    print(f"Hindi summary saved to: {hindi_filename}")
